...
```{r}
fecha_inicial <- make_date(2019, 3, 16)
fecha_cuarentena <- make_date(2020, 3, 16)
fecha_movilidad <- make_date(2020, 6, 30)
...
ranking_robos <-
  robos_cv %>% 
  group_by(mes_hechos, categoria_delito) %>% 
...
robos_cv %>% 
  # filter(categoria_delito %in% categorias_prioritarias) %>% 
  group_by(mes_hechos, categoria_delito) %>% 
  count(name = "total") %>% 
  ungroup() %>% 
  complete(mes_hechos, nesting(categoria_delito), fill = list(total = 0)) %>% 
  ggplot(aes(x = mes_hechos, y = total, group = categoria_delito)) +
  geom_line() +
  geom_smooth() +
  labs(title = "High impact theft trending by type",
       subtitle = "Monthly, December 2017 - June 2021",
       x = "Month",
       y = "Thefts") +
...
```{r}
distancia <- set_units(450, m)
tiempo <- period(3, "days")
```
...
puntos <-
  robos_cv_puntos %>%
  filter(between(fecha, fecha_inicial, fecha_cuarentena),
...
```{r}
clasificacion <- classIntervals(lineas_et_totales$nodos, n = 4, style = "jenks")
clasificacion
```
...
st_write(lineas_et_totales, 
         dsn = str_c("Salidas/lineas_et_rt_", 
                     make_clean_names(corte_inicio), 
                     "_", 
                     make_clean_names(corte_fin), 
                     ".gpkg"),
         delete_dsn = T)
```
...
leaflet() %>% 
  addTiles() %>% 
  addMeasure(primaryLengthUnit = "meter")
```
